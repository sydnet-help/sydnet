...
This time we visualise the distribution of one variable but do it by different groups. Suppose that we want to compare the distribution of _fechld_ between men and women. ‘`sjp.xtab(the name of variable to be compared, grouping variable, bar.pos = “stack”)`’ make this job easier. The following code shows the grouped stacked bar graph of _fechld_ by _sex_.
...
You may see a warning message saying <span style="color:red">"Package 'snakecase' needs to be installed for case-conversion"</span> when you try to make a grouped boxplot. To fix this problem, run the following code.

```{r eval=FALSE, echo=TRUE}
install.packages("snakecase")
```

Or you may see a warning message saying <span style="color:red">"there is no pacakge called 'quantreg'"</span>. In this case run the following code.
